...
/* What to do when timer interrupt occurs? In this case, we update "ticks",
   ...
   when the system gets initialized. (e.g. in "kernel.C") */
    ...
    Console::puts("EOQ handler.\n");
    if(Thread::CurrentThread()){
        SYSTEM_SCHEDULER->resume(Thread::CurrentThread());
        /*This port number 0x20 is the port for the interrupt controller.*/
        /*0x20 command is issued to the PIC controller to indicate end of an interrupt*/
...
void EOQTimer::current(unsigned long * _seconds, int * _ticks) {
/* Return the current "time" since the system started. */
...
void EOQTimer::wait(unsigned long _seconds) {
/* Wait for a particular time to be passed. This is based on busy looping! */
